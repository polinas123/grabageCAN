...
@TargetApi(21)
public class Camera2BasicFragment extends CameraPreviewFragment {
    ...
    boolean calledFromImageReader;

    boolean calledFromCaptureCompleted;
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener =
            new ImageReader.OnImageAvailableListener() {
                ...
                public void onImageAvailable(ImageReader reader) {
                    Log.d(TAG, "onImageAvailable: I think this is not called - or second");
                    try {
                        fileUsed.acquire();
    ...
    //
    //        Log.d(TAG, String.valueOf(useCameraBackFacing));
    //    }
    ...
    public static Camera2BasicFragment newInstance() {
        if (Build.VERSION.SDK_INT < 21) {
            Log.e(
                    TAG,
                    "API level is lower than the required 21 (it is: "
...
//        captureBuilder.set(CaptureRequest.FLASH_MODE,CameraMetadata.FLASH_MODE_OFF);
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            ...
            for (String cameraId : manager.getCameraIdList()) {
                ...
                // Skip if it's the wrong camera
                Integer facing = characteristics.get(CameraCharacteristics.LENS_FACING);
                if (facing != null
                        && ((facing == CameraCharacteristics.LENS_FACING_BACK && !useCameraBackFacing)
                        || (facing == CameraCharacteristics.LENS_FACING_FRONT && useCameraBackFacing)
            ...
            mCameraDevice.createCaptureSession(
                    ...
                    new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigureFailed(@NonNull CameraCaptureSession cameraCaptureSession) {
                            Log.e(TAG, "Failed configuration in createCameraPreviewSession");
                        }
                    },
                    null);
        } catch (CameraAccessException e) {
    ...
    private void captureStillPicture() {
        if (!isCapturing) {
            try {
                ...
                CameraCaptureSession.CaptureCallback captureCallback =
                        new CameraCaptureSession.CaptureCallback() {
                            ...
                            public void onCaptureCompleted(
                                ...
                                unlockFocus();
                                isCapturing = false;
                                calledFromCaptureCompleted = true;
                                captureFinishedWhenCalledTwice();
                            }
                ...
                calledFromImageReader = false;
                calledFromCaptureCompleted = false;
    ...
    private void setAutoFlashCapture() {
        if (mFlashSupported) {
            setFlashStatus();
        }
    }
    ...
    private void captureFinishedWhenCalledTwice() {
        if (calledFromImageReader && calledFromCaptureCompleted) {
            EventBus.getDefault().post(new SavedSourceFileMessage(mFile));
    ...
    static class CompareSizesByArea implements Comparator<Size> {
        ...
        public int compare(Size lhs, Size rhs) {
            if (minimumRatio > 0 && maximumRatio > 0) {
                double lratio = (double) lhs.getWidth() / lhs.getHeight();
                double rratio = (double) rhs.getWidth() / rhs.getHeight();
                if ((lratio < minimumRatio || lratio > maximumRatio)
                        && rratio > minimumRatio
                        && rratio < maximumRatio) {
                    return -1;
                } else if ((rratio < minimumRatio || rratio > maximumRatio)
                        && lratio > minimumRatio
                        && lratio < maximumRatio) {
                    return 1;
                }
